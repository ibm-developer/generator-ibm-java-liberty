...
const generate = function (docs, parentLogger) {
  ...
  logger.writeToLog('Liberty Generator generating code from open api document: ' + JSON.stringify(docs,null,2))
  const openApiDir = []
...
const generateFromDoc = function (doc) {
  return performSDKGenerationAsync('libertySDK', 'server_java_jaxrs_spec', doc)
    .then(generatedID => {
      logger.writeToLog('Liberty Generator generated code from open api document with id ' + generatedID)
      return getServerSDKAsync('libertySDK', generatedID)
    })
}

const writeFiles = function (dirs, generator) {
  dirs.forEach(sdk => {
    generator.fs.copy(path.join(sdk.tempDir, 'generated-code', 'JavaJaxRS-Spec', 'src', 'gen'), generator.destinationPath('src/main'))
    generator.fs.delete(generator.destinationPath('src/main/java/io/swagger/api/RestApplication.java'))
...
const performSDKGenerationAsync = function (sdkName, sdkType, fileContent) {
  ...
  return request.postAsync({
    ...
    .then(function (response) {
      ...
      logger.writeToLog('SDK generation job response body: ' + JSON.stringify(body, null, 2))
...
function getStatusAsync (generatedID) {
  ...
  return requestAsync({
    ...
    .then(response => {
      logger.writeToLog('SDK generation status response: ' + JSON.stringify(response, null, 2))
